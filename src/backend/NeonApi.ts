...
export class NeonApi {
    ...
    public async getPredictWithGemini(
        ...
        const from7month = new Date(
            currentDate.getFullYear(),
            currentDate.getMonth() -6
        );
        const getPrevMonth = (i : number) => {
            const currentDate = new Date();
            for(let j = 0; j < i; j++) {currentDate.setMonth(currentDate.getMonth()  - 1 )}
            return currentDate.getFullYear() +
                "-" +
                ("00" + (currentDate.getMonth() + 1)).slice(-2)};
        const last3Months = [0, 1, 2].map(i => getPrevMonth(i));
        const past4To6Months = [3, 4, 5].map(i => getPrevMonth(i));
        const month1 =
